...
clean-srcs-local::
	...
	$(Q)$(RM) $(PRISM_BUILD_DIR)/parsey/parse.c $(PRISM_BUILD_DIR)/parsey/parse.h $(PRISM_BUILD_DIR)/parsey/parse.c.i
...
realclean-srcs-local:: distclean-srcs-local
	$(Q)$(CHDIR) $(srcdir) && $(RM) \
	  ...
	  prism/parsey/parse.c prism/parsey/parse.h \
...
# The parse.y backend's parser is generated by lrama like the top-level
# parse.c, using the backend's own vendored id2token.rb and id.def.  The
# header lands next to it, where its bare #include "parse.h" resolves.
# Two steps rather than a pipe: a pipe reports only lrama's status, which
# happily parses a truncated grammar if the filter dies mid-stream.
$(PRISM_BUILD_DIR)/parsey/parse.c: $(PRISM_SRCDIR)/parsey/parse.y $(PRISM_SRCDIR)/parsey/defs/id.def $(PRISM_SRCDIR)/parsey/tool/id2token.rb $(PRISM_BUILD_DIR)/parsey/.time
	$(ECHO) generating $@
	$(Q)$(BASERUBY) $(PRISM_SRCDIR)/parsey/tool/id2token.rb $(PRISM_SRCDIR)/parsey/parse.y > $@.i
	$(Q)$(LRAMA) -o$@ -H$(PRISM_BUILD_DIR)/parsey/parse.h - $(PRISM_SRCDIR)/parsey/parse.y < $@.i
	$(Q)$(RM) $@.i
...
common-srcs: $(srcs_vpath)parse.c $(srcs_vpath)lex.c $(srcs_vpath)enc/trans/newline.c $(srcs_vpath)id.c \
	     $(PRISM_BUILD_DIR)/parsey/parse.c \
	     $(BUILTIN_RB_INCS) \
